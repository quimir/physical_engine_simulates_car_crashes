...
QImage FileReadSystem::ReadImageFile(const QString &file_path)
{
    ...
    if (!QFile::exists(file_path))
    {
        FileWriteSystem::GetInstance().OutMessage(
            ...
            file_path + " failed to open the file. Please check for details: "
    ...
    if (!image_reader.format().isEmpty())
    {
        ...
        FileWriteSystem::GetInstance().OutMessage(
            FileWriteSystem::MessageTypeBit::kDebug,
            "Image name: " +image_reader.fileName() +", picture format: " +
                QString::fromStdString(image_reader.format().toStdString()) +
                ", file width: "+ QString::number(image.width()) +", height: " +
...
QMap<QString, QMap<QString, QList<QString> > > FileReadSystem::ReadJsonFile(
    QFile json_file)
{
...
bool FileReadSystem::ReadFileFirstLine(const QString& file_path,
                                       QString& first_line)
{
    if(file_path.isEmpty())
    {
        FileWriteSystem::GetInstance().OutMessage(
            FileWriteSystem::MessageTypeBit::kDebug,"File path is null!");
        return false;
    }
    ...
    QMap<QString, QMap<QString, QList<QString> > > &json_map)
{
    ...
    for (const QString &key : keys)
    {
        ...
        {
            FileWriteSystem::GetInstance().OutMessage(
                FileWriteSystem::MessageTypeBit::kDebug, "Json key is null");
            continue;
        }
...
void FileReadSystem::ParseJsonObject(
    const QJsonObject& json_obj,
    QMap<QString, QList<QString>> &nested_map,const QString &parent_key,
    QList<QString>& list_value)
{
    QStringList keys = json_obj.keys();
...
void FileReadSystem::ParseJsonArray(
    const QJsonArray& json_array,
    QMap<QString,QList<QString>>& relative_paths,
...
QMap<QString, QMap<QString, QList<QString>>> FileReadSystem::ReadGLSLFile(
    ...
    resourcesfiletype::ResourcesType type)
{
    ...
    {
        FileWriteSystem::GetInstance().OutMessage(
            FileWriteSystem::MessageTypeBit::kDebug,"GLSL map is null!");
        return glsl_map;
    }
    ...
    while(it!=glsl_map.end())
    {
                ...
                Qt::CaseInsensitive))
        {
            ...
            for(const QString&key:keys)
            {
                ...
                {
                    QString file_code=FilePathSystem::GetInstance()
                                        .GetResourcesPath(type,relative_path);
